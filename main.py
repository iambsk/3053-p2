...
def main():
    ...
    backbone_port = 8001
    hub_thread = threading.Thread(target=BackboneHub, args=(backbone_port,))
    hub_thread.start()
    print("Starting switches")
    switches = []
    for i in range(3):
        switch_port = 8002 + i
        switch = Hub(switch_port, backbone_socket=socket.create_connection(('localhost', backbone_port)))
